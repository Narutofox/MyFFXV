...
namespace Gamekit3D
{
    ...
    public class SpitterBehaviour : MonoBehaviour, IMessageReceiver, IEnemyBehavior
    {
        ...
        private void MagicSpeedEffects()
        {
            if (m_Controller.Freezing)
            {
                m_Controller.animator.speed /= 2;
            }
            else if (m_Controller.Electrocuted)
            {
                m_Controller.animator.speed = 0;
            }
            else if (m_Controller.animator.speed < 1)
            {
                m_Controller.animator.speed = 1;
            }
        }
        ...
        {
            // Za pomicanje se koriste animacije a nav mesh je samo za navigaciju s obzirom da to ne razumijm u potpunosti neznam kako spriječiti lika da pobjegne krz planinu
